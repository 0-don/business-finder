...
export class GridGenerator {
  ...
  async generate(): Promise<number> {
    ...
    while (radius >= this.settings.minRadius) {
      ...
      console.log(
        `[${dayjs().format("HH:mm:ss")}] ${radius}m: ${insertedCount} circles (total: ${await this.repo.getTotalCount()}) - ${dayjs().from(this.startTime)}`
      );
  ...
  private async generateGridAtRadius(
    ...
    const dyDeg = (radius * 1.5 * 360) / 40008000; // lat spacing
